...
switch($method) {
    case 'GET':
        getFoodSubCategories($db);
        break;
    case 'POST':
        addFoodSubCategory($db);
        break;
    case 'PUT':
        updateFoodSubCategory($db);
        break;
    case 'DELETE':
        deleteFoodSubCategory($db);
        break;
    default:
...
function getFoodSubCategories($db) {
    try {
        ...
        sendResponse(true, 'Food sub-categories retrieved successfully', $subCategories);
    } catch(PDOException $exception) {
        sendResponse(false, 'Error: ' . $exception->getMessage());
    }
}

function addFoodSubCategory($db) {
    ...
    try {
        // Verify that the master category belongs to Food module
        $verify_query = "SELECT maste_cat_food_id FROM master_categories WHERE maste_cat_food_id = :id AND module = 'Food'";
        $verify_stmt = $db->prepare($verify_query);
        $verify_stmt->bindParam(':id', $master_cat_food_id);
        $verify_stmt->execute();
        
        if ($verify_stmt->rowCount() === 0) {
            sendResponse(false, 'Invalid master category ID or category does not belong to Food module');
            return;
        }
        ...
        if ($stmt->execute()) {
            ...
            sendResponse(true, 'Food sub-category added successfully', $new_sub_category);
        } else {
            sendResponse(false, 'Failed to add food sub-category');
        }
    } catch(PDOException $exception) {
        sendResponse(false, 'Error: ' . $exception->getMessage());
    }
...
function updateFoodSubCategory($db) {
    $data = json_decode(file_get_contents("php://input"), true);
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            sendResponse(true, 'Food sub-category updated successfully', $updated_sub_category);
        } else {
            sendResponse(false, 'Failed to update food sub-category');
        }
    } catch(PDOException $exception) {
        sendResponse(false, 'Error: ' . $exception->getMessage());
    }
...
function deleteFoodSubCategory($db) {
    $data = json_decode(file_get_contents("php://input"), true);
    ...
    try {
        ...
        if ($stmt->execute()) {
            sendResponse(true, 'Food sub-category deleted successfully');
        } else {
            sendResponse(false, 'Failed to delete food sub-category');
        }
    } catch(PDOException $exception) {
        sendResponse(false, 'Error: ' . $exception->getMessage());
    }
...
// Get master categories for Food module (for dropdown)
function getFoodMasterCategories($db) {
    try {
        $query = "SELECT maste_cat_food_id, name FROM master_categories WHERE module = 'Food' ORDER BY name ASC";
        $stmt = $db->prepare($query);
        $stmt->execute();
        ...
        sendResponse(true, 'Food master categories retrieved successfully', $categories);
    } catch(PDOException $exception) {
        sendResponse(false, 'Error: ' . $exception->getMessage());
    }
}
